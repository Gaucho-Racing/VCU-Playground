...
int state_1_on(int dimmer) { //on state
   cout << "Changing state to on…" << endl << "Turning on light…" << endl;
   //and we turn on the light…
   /*
      this is probably some highly inefficient solution… ._.
      loop:
         get current light dimmer value based on some math formula and data from light sensor
         set light to inputted dimmer value based on some math formula, or gradually if change is too much compared to current light dimmer value (set light dimmer to something less that's computed from another math formula, the rest of the "curve" will be dealt with through other iterations of the loop, assuming the input dimmer value doesn't change from there)
         check via sensor if light is running properly
         failure response, change state to -1 (via return to main) if necessary.
         check for state change inputs, change states (via return to main) if necessary.
         repeat with updated input dimmer value.
   */
}
...
int main(int argc, char const *argv[]) {
   ...
   This solution kind of breaks down when there are continous changes to the output within a
   certain "state" based on continous sensor input (ex: motor running based on accelerator 
   input), especially for the inital output when just entering a state.
   */
   while (state != 7) {
      switch (state) {
         case -1:
         ...
         case 0:
            ...
            break;
         case 1:
            //get dimmer input value?
            state = state_1_on(/*dimmer value*/ 100); //stub 100 for now…
            break;
         case 7:
            break;
      }
